...
import { useState, useEffect, useCallback, useImperativeHandle, forwardRef, cloneElement, isValidElement } from "react"
...
export const Flipbook = forwardRef<FlipbookRef, FlipbookProps>(({ 
  ...
  const goToFrame = useCallback((index: number) => {
    const clamped = Math.max(0, Math.min(index, animation.frames.length - 1))
  ...
  }, [animation.frames.length, fps, loop, pingPong, paused, direction, onComplete])
