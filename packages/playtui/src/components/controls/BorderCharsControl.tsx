...
export function BorderCharsControl({ value, onChange, focusedField, setFocusedField }: {
  ...
  return (
    <box style={{ flexDirection: "column", gap: 0, marginTop: 1 }}>
      <text fg={COLORS.muted} attributes={1 << 1}>Custom Characters</text>
      <box style={{ flexDirection: "row", flexWrap: "wrap", gap: 1, marginTop: 1 }}>
        {GRID_ORDER.map((key) => {
          const isFocused = focusedField === `border-char-${key}`
          return (
            <box key={key} style={{ flexDirection: "column", width: 4, gap: 0 }}>
              <text fg={COLORS.muted} attributes={1 << 1}>{CHAR_LABELS[key]}</text>
              <box 
      ...
      <box 
        ...
        style={{ marginTop: 1, paddingLeft: 1, paddingRight: 1, backgroundColor: COLORS.bgAlt }}
      >
