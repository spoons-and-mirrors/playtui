...
export function RenderPreviewModal({ code, onClose, width, height }: RenderPreviewModalProps) {
  useKeyboard((key) => {
      ...
      if (!result.success) {
        ...
        if (nodes.length === 0) {
          ...
          const previewRoot = {
            ...
            id: "preview-root",
            name: "Preview",
            children: nodes.filter(Boolean).map(n => ({ ...n, children: n.children || [] })),
          }
          const noop = () => {}
          renderedContent = <Renderer node={previewRoot} selectedId={null} hoveredId={null} onSelect={noop} onHover={noop} />
        }
      }
    }
  } catch (e) {
  ...
  return (
    ...
    >
      ...
      >
        {error ? (
          <box id="render-error" style={{ padding: 2, flexGrow: 1, alignItems: "center", justifyContent: "center" }}>
            <box style={{ flexDirection: "column", gap: 1, alignItems: "center" }}>
              ...
              <text fg={COLORS.muted} style={{ marginTop: 1 }}>Press ESC to close</text>
            </box>
          </box>
        ) : (
          <box id="render-viewport" style={{ padding: 1, flexGrow: 1, alignItems: "center", justifyContent: "center" }}>
            {renderedContent}
          </box>
        )}
        ...
        >
          <text fg={COLORS.muted} selectable={false}>Press ESC or click outside to close</text>
        </box>
      </box>
    </box>
  )
