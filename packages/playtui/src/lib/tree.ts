...
export function syncIdCounter(root: Renderable): void {
  ...
  const findMaxId = (renderable: Renderable): number => {
    ...
    const childMax = renderable.children.reduce(
      (max, c) => Math.max(max, findMaxId(c)),
      0,
    )
...
export function updateRenderable(
  root: Renderable,
  id: string,
  updates: Partial<Renderable>,
): Renderable {
  if (root.id === id) return { ...root, ...updates }
  return {
    ...root,
    children: root.children.map((c) => updateRenderable(c, id, updates)),
  }
}
...
export function addChild(
  ...
  return {
    ...root,
    children: root.children.map((c) => addChild(c, parentId, newChild)),
  }
}
...
export function countRenderables(renderable: Renderable): number {
  return (
    1 + renderable.children.reduce((sum, c) => sum + countRenderables(c), 0)
  )
}
...
export function moveRenderable(
  root: Renderable,
  renderableId: string,
  direction: 'up' | 'down',
): Renderable | null {
